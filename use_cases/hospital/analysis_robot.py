...
def generate(df):
    ...
    df.loc[:, "timestamp"] = pd.to_datetime(df["simTime"], unit="s")
    traces = df.groupby(['robotId', pd.Grouper(key='timestamp', freq='86400s')])['traces'].agg(list).reset_index()
    ...
    traces["traces"].apply(lambda x: x.append((x[-1][0] + 0.1, {"recover"})))
    return traces.drop(traces[traces['robotId'] == -1].index)
...
def simulate(path):
    ...
    tr["F0"] = tr.apply(lambda x: evaluate_robustness_semantics(x["traces"], formula0), axis=1)
    return tr["F0"].tolist()  # , tr["F1"].values
...
for f in files:
    ...
    if len(data) > 4:
        ...
        f0 = simulate(os.path.join(folder_path, f))
        cases.append([mech, topology, nb, nm, a, r, fp, f0])
        print([mech, topology, nb, nm, a, r, fp])
df = pd.DataFrame(cases, columns=["mech", "topology", "nb", "nm", "a", "r", "fp", "f0"])
df.to_csv("./results/all_robot.csv")

print("DONE -- saved in ./results/all_robot.csv ")